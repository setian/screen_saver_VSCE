...
var DEFAULT_REPOSITORY_IDS = SNIPPET_REPOSITORIES.map((repo) => repo.configId).filter((id) => typeof id === "string");
...
function getEnabledRepositoryIds() {
  const config = vscode.workspace.getConfiguration("screenSaver");
  const configured = config.get("repositories.enabled");
...
function queueLoadCodeDelivery(payload, reason, attempt = 0) {
  if (!webviewPanel) {
    pendingLoad = void 0;
    return;
  }
  const entry = { payload, reason };
  pendingLoad = entry;
  if (!webviewReady) {
    return;
  }
  ...
  webviewPanel.webview.onDidReceiveMessage(async (message) => {
    ...
    switch (message.command) {
      ...
      case "requestNewGist":
        ...
        if (webviewPanel) {
          currentTypingSpeed = vscode.workspace.getConfiguration("screenSaver").get("typingSpeed", 40);
...
function activate(context) {
  ...
  const resetIdleTimer = () => {
    ...
    let idleTime = configuration.get("idleTimeSeconds", 300);
...
async function showRepositoryPicker() {
  const enabledIds = getEnabledRepositoryIds();
  const items = SNIPPET_REPOSITORIES.map((repository) => ({
  ...
  const selectedIds = selection.map((item) => item.repository.configId).filter((id) => typeof id === "string");
  const configuration = vscode.workspace.getConfiguration("screenSaver");
